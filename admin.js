...
const AdminApp = {
    ...
    async editProduct(productId) {
        ...
        // المسمى الجديد للسعر قبل الخصم هو compare_price
        document.getElementById('product-sale-price').value = product.compare_price || '';
        document.getElementById('product-stock').value = product.stock;
        document.getElementById('product-sku').value = product.sku || '';
        
        // المسمى الجديد للتميز هو is_featured
        document.getElementById('product-featured').checked = product.is_featured || false;
        
        // المسمى الجديد للحالة هو is_active
        document.getElementById('product-active').checked = product.is_active !== false;
        
        // المسمى الجديد لتاريخ انتهاء الخصم هو discount_ends_at
        if (product.discount_ends_at) {
            document.getElementById('product-sale-end').value = product.discount_ends_at.slice(0, 16);
        }
    ...
    async handleProductSubmit() {
        ...
        const prodDesc = document.getElementById('product-description').value.trim();
        
        // 2. توليد Slug فريد (الاسم + رقم عشوائي) لضمان عدم التكرار في قاعدة البيانات
        const uniqueSlug = AdminUtils.generateSlug(prodName) + '-' + Math.floor(1000 + Math.random() * 9000);

        // 3. التأكد من وجود مصفوفة صور أو وضع صورة افتراضية لمنع اختفاء المنتج
        const finalImages = (AdminState.uploadedImages && AdminState.uploadedImages.length > 0) 
            ? AdminState.uploadedImages 
            : ['https://vhqrbrlosqnzpqtxhypc.supabase.co/storage/v1/object/public/products/no-image-placeholder.png'];

        // 4. بناء كائن البيانات النهائي بالمسميات الصحيحة لقاعدة البيانات (Database Mapping)
        const productData = {
            name: prodName,
            name_ar: prodName, // نسخ الاسم للحقل العربي
            slug: uniqueSlug,
            description: prodDesc,
            description_ar: prodDesc, // نسخ الوصف للحقل العربي
            ...
            images: finalImages,
            thumbnail: finalImages[0] // تعيين أول صورة كصورة مصغرة أساسية
        };
        
        // 5. في حالة التعديل (Update)، نحذف الـ Slug لكي لا يتغير رابط المنتج القديم وتنكسر روابط الأرشفة
        if (productId) {
            delete productData.slug;
        }

        // 6. التحقق النهائي من الحقول الإجبارية قبل الإرسال
        if (!productData.name || productData.price <= 0) {
            AdminToast.warning('يرجى إدخال اسم المنتج وسعر صحيح');
            return;
        }
    ...
    async loadCategoriesForSelect() {
        if (AdminState.categories.length === 0) {
            // البحث عن الحقل الصحيح في الداتابيز is_active
